...
const stageStyle = {
  "Meeting Booked": { background: "#258530", color: "#fff" },
  Active: { background: C.text, color: "#fff" },
  ...
  "Follow-up Due": { background: "#8b1a1a", color: "#fff" },
};

function StageTag({ stage }) {
  const style = stageStyle[stage] || { background: C.base, color: C.hint };
  return <span style={{ fontSize: 10, padding: "4px 9px", borderRadius: 999, fontWeight: 700, ...style }}>{stage}</span>;
}
        ...
        <div style={{ padding: "16px 14px 10px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
            {[{ label: "Total", value: contacts.length }, { label: "Investors", value: contacts.filter((item) => item.type === "Investor").length }, { label: "Research", value: contacts.filter((item) => item.type === "Research").length }, { label: "Prospects", value: contacts.filter((item) => item.type === "Prospect").length }].map((item) => (
              <div key={item.label} style={{ background: C.base, border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 11px" }}>
      ...
      {selected ? (
        <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", gap: 20, alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    ...
                    style={{ borderRadius: 12, border: "none", background: C.text, color: "#fff", padding: "10px 14px", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6 }}
                  >
                  ...
                  <button
                    ...
                    style={{ borderRadius: 12, border: `1px solid ${C.accentM}`, background: C.accentL, color: C.accent, padding: "10px 14px", fontSize: 12, fontWeight: 700 }}
                  >
                    Book call
                  </button>
                </div>
            ...
            <div style={{ padding: 24, overflowY: "auto", display: "grid", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10 }}>
                {[{ label: "Type", value: selected.type }, { label: "Stage", value: <StageTag stage={selected.stage} />, element: true }, { label: "Track", value: selected.track }, { label: "Assigned", value: selected.agent }].map((item) => (
                  <div key={item.label} style={{ background: C.base, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px" }}>
              ...
              <div style={{ background: C.base, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16 }}>
          ...
          <div style={{ width: 270, borderLeft: `1px solid ${C.border}`, background: C.surface, padding: 20, flexShrink: 0 }}>
            <div style={{ background: C.accentL, border: `1px solid ${C.accentM}`, borderRadius: 14, padding: 16, marginBottom: 12 }}>
            ...
            <button
              onClick={() => onSendEmail(selected)}
              style={{ width: "100%", borderRadius: 12, border: "none", background: C.text, color: "#fff", padding: "11px 14px", fontSize: 12, fontWeight: 700, marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
            >
              <Icon name="mail" size={13} color="#fff" />
              Send email
            </button>
            <button
              onClick={() => onBookMeeting(selected)}
              style={{ width: "100%", borderRadius: 12, border: `1px solid ${C.border}`, background: C.surface, color: C.text, padding: "11px 14px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
            >
